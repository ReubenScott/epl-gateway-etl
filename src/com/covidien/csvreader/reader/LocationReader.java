	...
	public void readLocationCSVFile(String path) {
		...
		try {
			...
			while( (location = beanReader.read(Location.class, header, processors)) != null ) {
				...
				nodeRevisions.append("(" + nid + "," + etlAdmin + ",'" +location.getLOCATION_ID().replace("'","\\'") + "','','','',unix_timestamp(),0),");
				node.append("("+vid+",'postal_address','','" + location.getLOCATION_ID().replace("'","\\'") + "'," + etlAdmin + ",'1',unix_timestamp(),unix_timestamp(),0,0,0),");
				countryNid = DBUtiltityFunctions.getCountryNid(stmt, countryCodeMap.get(location.getCOUNTRY_CODE()));

				if(location.getCOUNTRY_CODE()!= null && countryNid==0) {
					insertCountry(nid+1, vid+1, location.getCOUNTRY_CODE(), etlAdmin);
					countryNid = nid+1;
					countryCreate = true;
				}
				
				contentTypePostalAddress.append("(" + nid + "," + vid + ",'");
				if(location.getADDRESS_LINE1()!=null) {
					contentTypePostalAddress.append(location.getADDRESS_LINE1().replace("'","\\'") + "','");

				} else {
					contentTypePostalAddress.append("','");
				}
				if(location.getCITY()!=null) {
					contentTypePostalAddress.append(location.getCITY().replace("'","\\'") + "','" + location.getSTATE_PROVINCE() + "','" + location.getPOSTAL_CODE() + "'," +
							countryNid + "),");					
				} else {
					contentTypePostalAddress.append("','" + location.getSTATE_PROVINCE() + "','" + location.getPOSTAL_CODE() + "'," +
							countryNid + "),");
				}

				locationResponse.append("(" + nid + ",'" + location.getLOCATION_ID() + "','PASS'),");
